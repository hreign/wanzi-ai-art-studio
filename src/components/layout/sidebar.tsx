...
import { Image, Video, Sparkles, Settings, Moon, Sun, ChevronLeft, ChevronRight } from "lucide-react";
...
const NAV_GROUPS: { label: string; models: ModelId[] }[] = [
  {
    label: "图像生成",
    models: ["sensenova-u1-fast", "agnes-image-2.1-flash"],
  },
  {
    label: "视频生成",
...
const modelIcons: Record<string, React.ReactNode> = {
  "sensenova-u1-fast": <Sparkles className="h-[18px] w-[18px]" />,
  "agnes-image-2.1-flash": <Image className="h-[18px] w-[18px]" />,
  "agnes-video-v2.0": <Video className="h-[18px] w-[18px]" />,
};

export function Sidebar({ collapsed, onToggle, onSettingsClick, activeModel, onModelChange }: SidebarProps) {
  ...
  return (
    ...
    >
      {/* Brand */}
      <div className="flex items-center gap-2.5 px-4 h-14 border-b border-[var(--color-border-secondary)]">
            ...
            <div className="flex flex-col gap-1">
              {group.models.map((modelId) => {
                ...
                return (
                    ...
                    className={cn(
                      "flex items-center gap-2.5 px-3 h-10 rounded-[var(--radius-sm)] transition-all duration-200",
                      isActive
      ...
      <div className="border-t border-[var(--color-border-secondary)] p-2 flex flex-col gap-0.5">
        ...
        <button
          ...
          className={cn(
            "flex items-center gap-2.5 px-2.5 h-9 rounded-[var(--radius-sm)]",
