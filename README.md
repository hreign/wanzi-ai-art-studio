...
AI 驱动的图像与视频生成工坊，集成 SenseNova U1 Fast、Agnes Image 2.1 Flash 与 Agnes Video V2.0 三大模型，支持文生图、图生图、文生视频、图生视频等创作方式。

![Wanzi AI Art Studio Preview](docs/preview.png)

## ✨ 功能特性

- **多模型支持** — SenseNova U1 Fast（商汤文生图）、Agnes Image 2.1 Flash（文/图生图）、Agnes Video V2.0（文/图生视频）
- **图像生成** — 多尺寸、批量生成（最多 4 张）、URL / Base64 返回格式
- **视频生成** — 自定义帧数 / 帧率 / 画面尺寸，支持参考图片生成视频
- **图生图 / 图生视频** — 上传或粘贴图片 URL 作为参考
- **历史记录** — 生成结果本地持久化，最多保留 100 条，支持回看与管理
...
> 点击上方按钮即可一键将项目部署到 Vercel。部署完成后访问应用，在设置中填入对应模型的 API Key 即可开始使用。
...
3. 根据模型 API 规范在 `src/services/` 中扩展调用逻辑

## 🔒 安全说明

- API 请求通过 `/api/generations` 代理转发，避免浏览器 CORS 限制
- 代理仅允许转发到白名单域名（`token.sensenova.cn`、`api.sensenova.cn`、`apihub.agnes-ai.com`）
- 请求体大小限制为 10MB
